...
.PHONY: abigen
abigen: ## Regenerate iNFT abigen bindings from contracts/out (ABI + bytecode for DeployEraPersonaINFT)
	...
	jq -r '.bytecode.object' contracts/out/EraPersonaINFT.sol/EraPersonaINFT.json | sed 's/^0x//' > /tmp/era_inft.bin
	abigen --abi /tmp/era_inft.abi --bin /tmp/era_inft.bin --pkg bindings --type EraPersonaINFT \
